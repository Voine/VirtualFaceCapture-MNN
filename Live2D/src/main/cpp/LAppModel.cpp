    ...
    , _userTimeSeconds(0.0f)
{
    ...
    // 顔の座標更新によるアニメーション
    _targetPointManager = CSM_NEW CubismTargetPointEx();
...
LAppModel::~LAppModel()
{
    ...
    if (_modelSetting) {
        ...
        {
            LAppDelegate::GetInstance()->GetTextureManager()->ReleaseTexture(_textureIds[modelTextureNumber]);
        }
        _textureIds.Clear();

        delete _modelSetting;
    }
    CSM_DELETE(_targetPointManager);
}
...
void LAppModel::PreloadMotionGroup(const csmChar* group)
{
    ...
    for (csmInt32 i = 0; i < count; i++)
    {
        ...
        csmString motionName = _modelSetting->GetMotionFileName(group, i);
        path = _modelHomeDir + path;
        ...
        if (buffer)
        {
            ...
            JniBridgeC::OnLoadOneMotion(group, i, motionName.GetRawString()); // リポートモーションデータの名前
        }
        else
        {
            if (_debugMode)
            {
                LAppPal::PrintLog("[APP]load Motion: %s, failed, skip.", path.GetRawString());
            }
        }
...
void LAppModel::Update(LAppModelParameters modelParameters)
{
    const csmFloat32 deltaTimeSeconds = LAppPal::GetDeltaTime();
    _userTimeSeconds += deltaTimeSeconds;

    // 顔の座標更新
     _targetPointManager->Set(modelParameters.faceX, modelParameters.faceY, modelParameters.faceZ);

    _targetPointManager->Update(deltaTimeSeconds);
    const csmFloat32 faceX = _targetPointManager->GetX();
    const csmFloat32 faceY = _targetPointManager->GetY();
    const csmFloat32 faceZ = _targetPointManager->GetZ();
    ...
    if (modelParameters.stopMotion)
    {
        _motionManager->StopAllMotions();
    }

    if (modelParameters.resetMotion)
    {
        _model->LoadDefaultParameters(); // 初期セーブされた状態をロード
    }

    if (modelParameters.changeMotion) {
        if (_debugMode)
        {
            LAppPal::PrintLog("[APP]Apply Load Motion: %s, %d.", modelParameters.nextMotionGroup.c_str(), modelParameters.nextMotionIndex);
        }
        StartMotion(modelParameters.nextMotionGroup.c_str(), modelParameters.nextMotionIndex, PriorityForce);
    }
    ...
    if (modelParameters.changeExpression) {
        if (_debugMode)
        {
            LAppPal::PrintLog("[APP]try start expression: %s.", modelParameters.nextExpressionName.c_str());
        }
        SetExpression(modelParameters.nextExpressionName.c_str());
    }

    if (modelParameters.stopExpression) {
        if (_debugMode) {
            LAppPal::PrintLog("[APP]stop all expressions !");
        }
    ...
    if (!motionUpdated)
    {
        if (_eyeBlink != NULL && modelParameters.autoBlinkEyesEnabled)
        {
            // メインモーションの更新がないとき
    ...
    if (!modelParameters.autoBlinkEyesEnabled)
    {
        // 目の開閉、eg: ひゆりは0.0から1.2
        _model->AddParameterValue(_idParamEyeLOpen, modelParameters.eyeLOpen - 1.0f);
        _model->AddParameterValue(_idParamEyeROpen, modelParameters.eyeROpen - 1.0f);
    }

    // 口の変形、eg: ひゆりは-2.0から
    _model->AddParameterValue(_idParamMouthForm, modelParameters.mouthForm - 1.0f);
    // 口の開閉
    _model->AddParameterValue(_idParamMouthOpenY, modelParameters.mouthOpenY);
    ...
    //ドラッグによる変化
    //ドラッグによる顔の向きの調整
    _model->AddParameterValue(_idParamAngleX, faceX * 30); // -30から30の値を加える
    _model->AddParameterValue(_idParamAngleY, faceY * 30);
    _model->AddParameterValue(_idParamAngleZ, faceZ * 30);
    // _model->AddParameterValue(_idParamAngleZ, faceX * faceY * -30);

    //ドラッグによる体の向きの調整
    _model->AddParameterValue(_idParamBodyAngleX, faceX * 10); // -10から10の値を加える

    //ドラッグによる目の向きの調整
    if (modelParameters.manualEyeBallEnabled)
    {
        _model->AddParameterValue(_idParamEyeBallX, modelParameters.eyeBallX);
        _model->AddParameterValue(_idParamEyeBallY, modelParameters.eyeBallY);
    }
    else
    {
        _model->AddParameterValue(_idParamEyeBallX, faceX); // -1から1の値を加える
        _model->AddParameterValue(_idParamEyeBallY, faceY);
    }

    // 呼吸など
    if (_breath != NULL && modelParameters.breathEnabled)
    {
...
void LAppModel::SetupTextures()
{
    for (csmInt32 modelTextureNumber = 0; modelTextureNumber < _modelSetting->GetTextureCount(); modelTextureNumber++)
    {
        ...
        if (texture)
        {
            ...
            _textureIds.PushBack(glTextueNumber);

            if (_debugMode)
            {
                LAppPal::PrintLog("[APP]bind texture: %d to %d", glTextueNumber, modelTextureNumber);
            }
        }
        else
        {
            if (_debugMode)
            {
                LAppPal::PrintLog("[APP]load texture file: %s, failed, skip.", texturePath.GetRawString());
            }
        }
    }
