...
#include "CubismTargetPointEx.hpp"
...
CubismTargetPointEx::CubismTargetPointEx()
                                    : _faceTargetX(0.0f)
                                    , _faceTargetY(0.0f)
                                    , _faceTargetZ(0.0f)
                                    , _faceX(0.0f)
...
void CubismTargetPointEx::Update(csmFloat32 deltaTimeSeconds)
{
    // デルタ時間を加算する
    ...
    const csmFloat32 dz = _faceTargetZ - _faceZ;

    if (CubismMath::AbsF(dx) <= Epsilon && CubismMath::AbsF(dy) <= Epsilon && CubismMath::AbsF(dz) <= Epsilon)
    {
        return; // 変化なし
    }

    // 速度の最大よりも大きい場合は、速度を落とす
    const csmFloat32 d = CubismMath::SqrtF((dx * dx) + (dy * dy) + (dz * dz));

    // 進行方向の最大速度ベクトル
    const csmFloat32 vx = MaxV * dx / d;
    const csmFloat32 vy = MaxV * dy / d;
    const csmFloat32 vz = MaxV * dz / d;

    // 現在の速度から、新規速度への変化（加速度）を求める
    csmFloat32 ax = vx - _faceVX;
    csmFloat32 ay = vy - _faceVY;
    csmFloat32 az = vz - _faceVZ;

    const csmFloat32 a = CubismMath::SqrtF((ax * ax) + (ay * ay) + (az * az));
    ...
    // ※本来、人間は筋力で力（加速度）を調整できるため、より自由度が高いが、簡単な処理ですませている
    {
        ...
        const csmFloat32 curV = CubismMath::SqrtF((_faceVX * _faceVX) + (_faceVY * _faceVY) + (_faceVZ * _faceVZ));
...
void CubismTargetPointEx::Set(csmFloat32 x, csmFloat32 y, csmFloat32 z)
{
    this->_faceTargetX = x;
    this->_faceTargetY = y;
    this->_faceTargetZ = z;
...
csmFloat32 CubismTargetPointEx::GetX() const
{
    return this->_faceX;
}

csmFloat32 CubismTargetPointEx::GetY() const
{
    return this->_faceY;
}

csmFloat32 CubismTargetPointEx::GetZ() const
{
    return this->_faceZ;
}
