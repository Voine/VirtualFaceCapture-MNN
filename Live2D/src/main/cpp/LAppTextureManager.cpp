...
LAppTextureManager::TextureInfo* LAppTextureManager::CreateTextureFromPngFile(std::string fileName)
{
    ...
    if (address)
    {
        ...
        LAppPal::PrintLog("[APP]glGetError after glTexImage2D: %d", glGetError());
        ...
        LAppPal::PrintLog("[APP]Texture Info: SourceFileName [%s] Width [%d] Height [%d] Channels [%d]", fileName.c_str(), width, height, channels);
...
{
    std::vector<Csm::csmUint32> textureIds;
    for (Csm::csmUint32 i = 0; i < _textures.GetSize(); i++)
    {
        textureIds.push_back(_textures[i]->id);
        delete _textures[i];
    }
...
void LAppTextureManager::ReleaseTexture(Csm::csmUint32 textureId)
{
    for (Csm::csmUint32 i = 0; i < _textures.GetSize(); i++)
    {
        ...
        Csm::csmUint32 textureId[] = { _textures[i]->id };
...
Live2D::Cubism::Framework::csmVector<LAppTextureManager::TextureInfo *> LAppTextureManager::GetAllTextures() {
    return _textures;
}
